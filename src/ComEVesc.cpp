...
ComEVesc::ComEVesc(uint32_t timeout_ms) : _TIMEOUT(timeout_ms) {
	nunchuck.valueX         = 127;
	nunchuck.valueY         = 127;
...
void ComEVesc::setSerialPort(Stream* port)
{
	serialPort = port;
}

void ComEVesc::setDebugPort(Stream* port)
{
	debugPort = port;
}
...
bool ComEVesc::unpackPayload(uint8_t * message, int lenMes, uint8_t * payload) {
...
int ComEVesc::packSendPayload(uint8_t * payload, int lenPay) {
...
bool ComEVesc::processReadPacket(uint8_t * message) {
...
bool ComEVesc::getFWversion(void){
	return getFWversion(0);
}

bool ComEVesc::getFWversion(uint8_t canId){
...
bool ComEVesc::getVescValues(void) {
	return getVescValues(0);
}

bool ComEVesc::getVescValues(uint8_t canId) {
...
void ComEVesc::setNunchuckValues() {
	return setNunchuckValues(0);
}

void ComEVesc::setNunchuckValues(uint8_t canId) {
...
void ComEVesc::setCurrent(float current, uint8_t canId) {
	int32_t index = 0;
...
void ComEVesc::setBrakeCurrent(float brakeCurrent) {
	return setBrakeCurrent(brakeCurrent, 0);
}

void ComEVesc::setBrakeCurrent(float brakeCurrent, uint8_t canId) {
	int32_t index = 0;
	int payloadSize = (canId == 0 ? 5 : 7);
	uint8_t payload[payloadSize];
	if (canId != 0) {
...
void ComEVesc::setRPM(float rpm) {
	return setRPM(rpm, 0);
}

void ComEVesc::setRPM(float rpm, uint8_t canId) {
	int32_t index = 0;
	int payloadSize = (canId == 0 ? 5 : 7);
	uint8_t payload[payloadSize];
	if (canId != 0) {
...
void ComEVesc::setDuty(float duty) {
	return setDuty(duty, 0);
}

void ComEVesc::setDuty(float duty, uint8_t canId) {
	int32_t index = 0;
	int payloadSize = (canId == 0 ? 5 : 7);
	uint8_t payload[payloadSize];
	if (canId != 0) {
...
void ComEVesc::sendKeepalive(void) {
	return sendKeepalive(0);
}

void ComEVesc::sendKeepalive(uint8_t canId) {
	int32_t index = 0;
	int payloadSize = (canId == 0 ? 1 : 3);
	uint8_t payload[payloadSize];
	if (canId != 0) {
...
void ComEVesc::serialPrint(uint8_t * data, int len) {
	if(debugPort != NULL){
		for (int i = 0; i <= len; i++)
		{
